...
Page({
      ...
      success: function (res) {
        if (res.status == 0) {
          ...
          for (let i = 0; i < res.data.length; i++) {
            ...
            if (idata.content.type == 2 && idata.content.url.article.type == 3) {
  ...
  turnToCommunityPublish : function(event) {
    ...
    let franchiseeParam = this.data.franchisee ? ('&franchisee=' + this.data.franchisee) : '';
    app.turnToPage('/informationManagement/pages/communityPublish/communityPublish?detail=' + this.data.communityId + '&articleId=' + id + '&type=' + type + franchiseeParam);
